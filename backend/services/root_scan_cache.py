...
class RootScanCacheService:
    def __init__(self) -> None:
        ...
        self._root_files: dict[str, float | None | list[str]] = {"ts": None, "items": []}
        self._root_folders: dict[str, float | None | list[str]] = {"ts": None, "items": []}
    ...
    def clear(self) -> None:
        with self._lock:
            self._root_files = {"ts": None, "items": []}
            self._root_folders = {"ts": None, "items": []}

    def _get_cached_items(
        self,
        entry: dict[str, float | None | list[str]],
        cache_ttl: float,
        ...
        with self._lock:
            cached_at = entry["ts"]
            if cached_at is not None and cache_ttl > 0 and now - float(cached_at) < cache_ttl:
