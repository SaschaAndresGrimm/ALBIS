...
- A TIFF written by **Series Operations** carries a header at all. That service has its own writer and called `write_tiff(path, arr)` with no metadata — its dependency's type did not even accept any — so summing a PILATUS series produced a TIFF stating only that ALBIS made it. It now borrows the export service's metadata readers rather than keeping a second copy, and the summed TIFF states the detector model, serial and location, pixel size, sensor thickness, tau, threshold setting, gain, wavelength, incident energy, detector distance and beam centre.
  ...
  The per-exposure fields are deliberately dropped rather than carried: a sum of ten one-second frames is not a one-second exposure, its values can exceed the per-frame count cutoff, and it spans a wedge of rotation rather than a single step. So `Exposure_time`, `Exposure_period`, `Count_cutoff`, `Start_angle`, `Angle_increment` and the frame timestamp are omitted, and the header says which operation combined how many frames and that those fields are absent because they do not describe a combined frame — silence would have read as "unknown" rather than "not applicable".

- Exported TIFFs carry the same header as exported CBFs. The `ImageDescription` tag held only the three provenance lines; it now holds the full header text, so a TIFF states the detector model, serial and location, the timestamp, pixel size, sensor thickness, tau, count cutoff, threshold setting, gain and the rotation angles — none of which the DECTRIS private tag has a slot for. They went into the standard text tag rather than new private tag ids, because those ids are DECTRIS's numbering and a collision with a real assignment would have their software misread ALBIS's files; `tifffile`, PIL and ImageJ all surface `ImageDescription`. Reading a TIFF back now merges both sources instead of returning whichever answered first — the private tag wins any field they share, and the text supplies the rest, where before picking one threw away the other half.

- A summed or averaged HDF5 output keeps the source's detector and beam metadata. `_copy_h5_metadata` copied a group's *attributes* and not its datasets, so everything a NeXus writer states as a dataset was dropped: the detector description, `sensor_thickness`, `count_time`, `frame_time`, `saturation_value`, and the beam's `incident_wavelength`. Scalar and short datasets under `/entry/instrument/detector` and `/entry/instrument/beam` are now copied with their `units`, while the pixel mask and any bulk array are left behind — a summed output has its own masking applied, and the mask describes the source frames. Geometry corrected in ALBIS is still written afterwards and still wins over the source's stale copy.
...
- Golden decode tests over the real detector files in `testdata/`. Ten PILATUS CBFs and a DECTRIS EIGER2 TIFF have sat in the repository unread while every other fixture in the suite was built by numpy and h5py, so nothing exercised a byte-offset stream or a private DECTRIS TIFF tag — the two places a dependency bump could change a displayed number without changing a line of ALBIS. The CBF pixels are now checked against a reference byte-offset decoder written from the specification, its metadata against the numbers in the file's own header text, and its integrity against the `Content-MD5` the file carries; the TIFF's private-IFD fields are pinned. A one-row shift, a transposed axis and a dropped sign were each confirmed to fail the new tests.
...
- The SIMPLON address and API version can no longer steer the request off the SIMPLON API. Both are interpolated into the outgoing request path. The version was only trimmed of leading and trailing slashes, so an interior `../` survived and `1.8.0/../../../admin/shutdown?x=` reached `/admin/shutdown` on the detector once the caller appended its own suffix; it is now required to be a dotted number. The address kept the same power through a `..` segment, and a query string or fragment swallowed the appended path entirely, sending the request to `/` instead. A path prefix is still accepted, for a detector behind a reverse proxy; a traversal segment, query, fragment or embedded credentials are not.

- A release from a tag refuses to publish unsigned Windows or macOS artifacts. The Authenticode check only ran `if ($windowsSigningConfigured)` and the macOS verification step was gated on the very secrets it polices, so an empty credential set skipped both checks rather than failing them — a dispatch build confirmed it, producing an unsigned Windows installer on a green run while README.md tells users the builds are signed. Credentials remain optional for `workflow_dispatch`, so a fork can still build unsigned.
...
- Keyboard focus can move again. `Tab` was bound to play/pause with an unconditional `preventDefault()`, evaluated before the guard that exempts form fields and with no check for `Shift`, so focus could not reach any of the roughly 265 controls in the main window and `Shift+Tab` did nothing either — a WCAG 2.1.1 failure across the whole interface. `Tab` still starts and stops playback, but only when focus is not already on a control, which is the case whenever the image itself has focus. The keyboard reference in README.md now states that condition.

- The focus indicator is visible. `--focus-ring` was a 34%-opacity accent that composited to 1.83:1 against the side panels and 1.39:1 against the toolbar, against the 3:1 that WCAG 1.4.11 asks — and it is the only focus affordance for around thirty control classes, each of which suppresses the native outline. Full-opacity accent alone still measures only 2.49:1 on the toolbar blue, so the indicator is now a dark inner ring inside an accent outer ring, whose own edge measures 7.42:1 and travels with it whatever sits behind. Two controls that carried their own weaker rings now use the shared token, and the analysis switches — a zero-sized transparent checkbox behind a styled track — show a ring on the track.

- The three analysis feature switches say what they are. Each is a `<label>` containing only the checkbox and a decorative track, so the accessible name computed to nothing and a screen reader announced the on/off control for ROI statistics, resolution rings and peak finding as an unnamed checkbox. They now take their name from the section title, translated in all thirteen languages.

- A collapsed panel section no longer holds keyboard focus. Collapsing hides the content with `max-height: 0` alone, so its controls stayed in the tab order while `aria-hidden="true"` told assistive technology they were not there; focus could land in a zero-height clipped region on a node screen readers had been told to ignore.

- The documented way to run ALBIS from source works. `python backend/app.py` was the first executable line of CONTRIBUTING.md's quick start and appeared in six places across three documents, and it had been broken for roughly seven releases: running a module of a package as a script leaves it without a parent package, so its relative imports fail before anything is served. Nothing that ships was affected — the launcher, the Docker image and the PyInstaller bundle all import `backend` as a package — which is precisely why nothing caught it. The quick starts now use `python albis_launcher.py`, with `python -m uvicorn backend.app:app` documented for the backend alone, and `python -m backend.app` works as well now that its Uvicorn target names the module rather than a bare `app:app` that only resolved with `backend/` on `sys.path`. The dual-import scheme that existed to support the broken command — eleven `try`/`except ImportError` blocks and 239 lines of duplicate imports, each marked `# pragma: no cover` and therefore invisible to the coverage gate — is gone, leaving one import path per module. New subprocess tests launch each documented command and wait for `/api/health`, which is the only way this class of breakage is visible: every other test imports the app object directly.
...
- The checks CONTRIBUTING.md asks contributors to run are the checks CI runs, and a test now holds them to it. The documented pytest command did not run at all: it set `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1` without `-p pytest_cov`, so pytest exited with `unrecognized arguments: --cov=backend`. It also gated coverage at 50 against CI's 77, and its ruff invocation omitted `albis_launcher.py`. The block had already been hand-patched twice and drifted again, so the parity is now asserted against `ci.yml` rather than maintained by hand.
...
- The rest of the File menu says what it can and cannot do. Close File, Save As ▸ Full Image, Save As ▸ Visible Area and Convert Dataset are greyed out when they cannot run, each carrying the reason on hover; the command palette already left these commands out in that state through its `when:` predicates, and the menu simply had no notion of availability. Two of them were worse off than Export Animation above: `exportFullImage` and `exportVisibleArea` returned early without setting any status at all, so ⌘S and ⇧⌘S with nothing open did nothing and said nothing. They now explain themselves through `status.export.no_image`, new in all thirteen locales, and both gate on `state.hasFrame` rather than `state.dataRaw` alone — `hasFrame` is what the rest of the app means by "a frame is on screen", and dataRaw outlives a failed frame load and an HDF5 dataset rescan, so saving off it could write out the previous frame's pixels while the splash was up. Save As ▸ Viewer Window stays available throughout, because it screenshots the page and does that perfectly well with no image open.

- One definition per rule behind the three doors onto the file commands. Whether a command can run was decided in up to three places — the command palette's `when:` predicates, each export controller's own guard, and the File menu greying added above — and the data-export rule was already written out twice, identically, in the palette and its controller. The rules now live once in `frontend/modules/command_availability.js` (`canSaveImage`, `canExportAnimation`, `canExportData`), and the palette, the menu and the controllers all read them from there. Two consequences worth naming: the palette gated the three image saves on `hasFrame` alone, so it now also withholds them for the moment during an HDF5 dataset rescan when a stale frame buffer is all that remains; and Save As ▸ Viewer Window, which the menu left enabled because a screenshot of the splash technically succeeds, is now refused with the other two, which is what the palette always did. Save As itself greys out with its three children instead of opening a submenu of dead entries.
...
- The greyed-out controls can be asked why. Marking a control unavailable with the native `disabled` attribute puts the explanation out of reach: a disabled element receives no pointer events, so neither the reason on hover nor the guard that would turn a click down out loud can fire — it just sits there dead, which is the dead end the greying was meant to replace. On the ROI export the reason was buried twice over, because the help tooltip reads `data-help` ahead of `title` and then strips `title` from everything it maps. Controls whose unavailability carries an explanation are now marked the way the File menu already marked its entries — `is-disabled` plus `aria-disabled`, with the reason in `data-help-reason`, which the tooltip reads first and the managed hints cannot overwrite. They stay hoverable and clickable: hovering says why, and clicking reaches the guard that says the same thing out loud. The native attribute stays for controls that are simply inert with nothing to say. The mask and saturation checkboxes had the same problem and keep their native `disabled` — a checkbox that looks unavailable but still toggles is worse — so their hint moved to the label around them, where it can be read at all; on the checkbox it never appeared.
