...
- Sync `THIRD_PARTY_LICENSES.md` with the actually-shipped dependency versions:
  - The version column must match `backend/requirements.txt` (and the vendored frontend libs). `tests/test_shipped_dependency_parity.py` enforces the Python side, including that the Dockerfile installs the pinned versions rather than its own.
  - Add/remove entries if dependencies changed, and verify no new copyleft (GPL/AGPL) dependency was introduced.
